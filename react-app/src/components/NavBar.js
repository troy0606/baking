...
import { GetCartData_Post, DelCartData_Post, InsertOrderData } from '../pages/cart/Actions'
                  ...
                  <Link to="/teacher/">玩樂烘培</Link>
                                ...
                                <li className="login-pos">
                                    ...
                                    {MemberLogState.loginStatus ? (
                                        <ul className="member-login" style={memberOpen ? {} : { display: 'none' }}>
                                            <li>
                                            ...
                                            <li onClick={() => dispatch(LOGOUT_ACTION(MemberLogState.memberSid))}>
                                ...
                                <li className="smell-cart">
                                    ...
                                    >
                                        <ul>
                                            {CartData.length == 0 && (
                                                <li className="d-flex justify-content-center align-items-center flex-column h-100">
                                                    {MemberLogState.loginStatus == true ? '沒有商品' : '尚未登入'}
                                                </li>
                                            )}
                                            {CartData.map((v, key) => {
                                                cartTotal = cartTotal + v.product_quantity * v.product_price
                                                return (
                                                    <li className="d-flex justify-content-around" key={key}>
                                                        <div className="smell-cart-left">
                                                            <img src={`/images/products/${v.product_img_1}`} alt="" />
                                                        </div>
                                        ...
                                        <div className="smell-cart-bottom d-flex justify-content-around">
                                            ...
                                            <span>
                                                <Link onClick={() => clearOrderData()}>前往結帳頁面</Link>
                                            </span>
                                        </div>
                                        <div className="smell-cart-close" onClick={() => setCartOpen(false)}>
                                            x
                                        </div>
                                    </div>
                                </li>
                        ...
                        <li className="rwd-menu">
                            <AiOutlineMenu onClick={() => setHamburgerMenu(!hamburgerMenu)} />
                        </li>
                    </ul>
                    {hamburgerMenu ? (
                        <ul className="hamburgerMenu">
                            ...
                            <li>
                                <Link onClick={() => clearOrderData()}>前往購物車頁面</Link>
                            </li>
                        </ul>
                    ) : (
                        ''
