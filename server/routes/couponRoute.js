...
class coupon_data {
    constructor(member_sid, coupon_sid, coupon_number) {
        ;(this.member_sid = member_sid), (this.coupon_sid = coupon_sid), (this.coupon_number = coupon_number)
    }
    getCouponData() {
...
router.post('/selectMemberCoupon', (req, res) => {
    let data = new coupon_data(req.body.member_sid, req.body.coupon_sid, req.body.coupon_number)
    console.log(req.body)
    console.log(data.selectMemberCouponData())
    db.query(data.selectMemberCouponData(), (error, rows) => {
        console.log(rows.length)
        if (rows.length > 0) {
            if (rows[0].member_coupon_used == 1) res.json({ status: '202', info: '已被使用', data: rows[0] })
